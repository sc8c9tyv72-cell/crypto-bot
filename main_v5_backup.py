...
def format_directional_analysis(result: dict, session_name: str) -> str:
    ...
    # SL：強制最小距離 = max(ATR×1.5, 0.5%)，優先用 Swing Low
    bull_sl, bull_sl_desc = find_sl_anchor_zone(
        bull_entry, "bullish", zones_15m, zones_4h, highs_15m, lows_15m, atr_15m)
    min_dist = max(price * 0.005, atr_15m * 1.5)
    if bull_sl >= bull_entry or abs(bull_entry - bull_sl) < min_dist:
        sl_swing = min([l[1] for l in lows_15m if l[1] < bull_entry], default=bull_entry - min_dist)
        bull_sl = min(sl_swing * 0.999, bull_entry - min_dist)
        bull_sl_desc = "15M Swing Low 外"
    bull_sl_dist = abs(bull_entry - bull_sl)

    # TP：上方 BSL / 1H OB 供應區，強制 RR ≥ MIN_RR
    bull_tp = None
    bull_tp_label = ""
    if bsl and bsl > bull_entry and abs(bsl - bull_entry) >= bull_sl_dist * MIN_RR:
        bull_tp = bsl
        bull_tp_label = "BSL 上方流動性"
    if not bull_tp:
        above_resist = sorted(
            [z for z in zones_1h if z.get('direction') == 'bearish'
             and z.get('mid', 0) > bull_entry
             and abs(z.get('mid', 0) - bull_entry) >= bull_sl_dist * MIN_RR],
            key=lambda z: z.get('mid', 0))
        if above_resist:
            bull_tp = above_resist[0]['mid']
            bull_tp_label = above_resist[0].get('label', '1H 供應區')
    if not bull_tp or bull_tp <= bull_entry:
        bull_tp = bull_entry + bull_sl_dist * MIN_RR
        bull_tp_label = f"1:{MIN_RR:.0f} RR 目標"
    bull_rr = abs(bull_tp - bull_entry) / bull_sl_dist if bull_sl_dist > 0 else 0
    ...
    if bear_sl <= bear_entry or abs(bear_entry - bear_sl) < min_dist:
        sl_swing = max([h[1] for h in highs_15m if h[1] > bear_entry], default=bear_entry + min_dist)
        bear_sl = max(sl_swing * 1.001, bear_entry + min_dist)
        bear_sl_desc = "15M Swing High 外"
    bear_sl_dist = abs(bear_entry - bear_sl)

    bear_tp = None
    bear_tp_label = ""
    if ssl and ssl < bear_entry and abs(bear_entry - ssl) >= bear_sl_dist * MIN_RR:
        bear_tp = ssl
        bear_tp_label = "SSL 下方流動性"
    if not bear_tp:
        below_support = sorted(
            [z for z in zones_1h if z.get('direction') == 'bullish'
             and z.get('mid', 0) < bear_entry
             and abs(bear_entry - z.get('mid', 0)) >= bear_sl_dist * MIN_RR],
            key=lambda z: z.get('mid', 0), reverse=True)
        if below_support:
            bear_tp = below_support[0]['mid']
            bear_tp_label = below_support[0].get('label', '1H 需求區')
    if not bear_tp or bear_tp >= bear_entry:
        bear_tp = bear_entry - bear_sl_dist * MIN_RR
        bear_tp_label = f"1:{MIN_RR:.0f} RR 目標"
    bear_rr = abs(bear_entry - bear_tp) / bear_sl_dist if bear_sl_dist > 0 else 0
...
def format_limit_order(result: dict) -> str:
    """
    掛單建議：主力用 1H 結構決定方向
    - 入場：當前價格 ±3% 內的 OB，且必須距現價 ≥ 0.5%
    - SL：強制最小距離 = max(ATR×1.5, 0.5%)
    - TP1：15M 關鍵位，強制 RR ≥ 1:2
    - TP2：1H 關鍵位，強制 RR ≥ 1:3
    """
    symbol    = result["symbol"]
    sym_short = symbol.replace("USDT", "/USDT")
    price     = result["price"]
    ...
    dir_text  = "做多" if direction == "bullish" else "做空"
    conf_emoji = "✅" if aligned else "🟡"
    confidence = "高勝率（4H + 1H 同向）" if aligned else f"注意 4H {struct_4h}，尚未與 1H 同步"

    # SL 最小距離標準
    min_sl_dist = max(price * 0.005, atr_15m * 1.5)

    # ── 掛單入場位：距現價 ≥ 0.5%，在 3% 範圍內 ──────────────────
    min_entry_dist = price * 0.005  # 入場位必須距現價至少 0.5%

    if direction == "bullish":
        entry_zones = sorted(
            [z for z in zones_15m + zones_1h
             if z.get('direction') == 'bullish'
             and z.get('mid', 0) < price - min_entry_dist   # 必須在現價下方 0.5% 以上
             and z.get('mid', 0) > price * 0.97],           # 不超過 3% 回調
            key=lambda z: abs(z.get('mid', 0) - price))

        if not entry_zones:
            return f"⚠️ {sym_short} 當前價格附近無合適掛單位置\n（需要現價下方 0.5%-3% 的看漲 OB，目前無符合條件的區域）\n建議：等待市場回調後再查詢，或直接市價入場。"
        ...
        if sl >= entry_price or abs(entry_price - sl) < min_sl_dist:
            sl_swing = min([l[1] for l in lows_15m if l[1] < entry_price], default=entry_price - min_sl_dist)
            sl = min(sl_swing * 0.999, entry_price - min_sl_dist)
            sl_desc = "15M Swing Low 外"
        if 0 <= now.hour < 8:
            sl -= price * 0.002
            sl_desc += "（含夜間加寬）"
        sl_dist = abs(entry_price - sl)
        ...
        tp1 = None; tp1_label = ""
        above_15m = sorted(
            [z for z in zones_15m if z.get('direction') == 'bearish'
             and z.get('mid', 0) > entry_price
             and abs(z.get('mid', 0) - entry_price) >= sl_dist * 2.0],
            key=lambda z: z.get('mid', 0))
        if above_15m:
            tp1 = above_15m[0]['mid']
            tp1_label = above_15m[0].get('label', '15M 供應區')
        if not tp1:
            above_eqh = sorted([e["price"] for e in eqh
                                 if e["price"] > entry_price + sl_dist * 2.0])
            if above_eqh:
                tp1 = above_eqh[0]; tp1_label = "15M EQH"
        if not tp1:
            tp1 = entry_price + sl_dist * 2.0; tp1_label = "1:2 RR 目標"

        # TP2：1H 關鍵位，強制 RR ≥ 1:3
        tp2 = None; tp2_label = ""
        if bsl and bsl > entry_price + sl_dist * 3.0:
            tp2 = bsl; tp2_label = "BSL 上方流動性"
        if not tp2:
            above_1h = sorted(
                [z for z in zones_1h if z.get('direction') == 'bearish'
                 and z.get('mid', 0) > entry_price + sl_dist * 3.0],
                key=lambda z: z.get('mid', 0))
            if above_1h:
                tp2 = above_1h[0]['mid']; tp2_label = above_1h[0].get('label', '1H 供應區')
        if not tp2:
            tp2 = entry_price + sl_dist * 3.5; tp2_label = "1:3.5 RR 目標"
        ...
        entry_zones = sorted(
            [z for z in zones_15m + zones_1h
             if z.get('direction') == 'bearish'
             and z.get('mid', 0) > price + min_entry_dist   # 必須在現價上方 0.5% 以上
             and z.get('mid', 0) < price * 1.03],
            key=lambda z: abs(z.get('mid', 0) - price))

        if not entry_zones:
            return f"⚠️ {sym_short} 當前價格附近無合適掛單位置\n（需要現價上方 0.5%-3% 的看跌 OB，目前無符合條件的區域）\n建議：等待市場反彈後再查詢，或直接市價入場。"
        ...
        if sl <= entry_price or abs(entry_price - sl) < min_sl_dist:
            sl_swing = max([h[1] for h in highs_15m if h[1] > entry_price], default=entry_price + min_sl_dist)
            sl = max(sl_swing * 1.001, entry_price + min_sl_dist)
            sl_desc = "15M Swing High 外"
        if 0 <= now.hour < 8:
            sl += price * 0.002
            sl_desc += "（含夜間加寬）"
        sl_dist = abs(entry_price - sl)
        ...
        tp1 = None; tp1_label = ""
        below_15m = sorted(
            [z for z in zones_15m if z.get('direction') == 'bullish'
             and z.get('mid', 0) < entry_price - sl_dist * 2.0],
            key=lambda z: z.get('mid', 0), reverse=True)
        if below_15m:
            tp1 = below_15m[0]['mid']; tp1_label = below_15m[0].get('label', '15M 需求區')
        if not tp1:
            below_eql = sorted([e["price"] for e in eql
                                 if e["price"] < entry_price - sl_dist * 2.0], reverse=True)
            if below_eql:
                tp1 = below_eql[0]; tp1_label = "15M EQL"
        if not tp1:
            tp1 = entry_price - sl_dist * 2.0; tp1_label = "1:2 RR 目標"

        # TP2：1H 關鍵位，強制 RR ≥ 1:3
        tp2 = None; tp2_label = ""
        if ssl and ssl < entry_price - sl_dist * 3.0:
            tp2 = ssl; tp2_label = "SSL 下方流動性"
        if not tp2:
            below_1h = sorted(
                [z for z in zones_1h if z.get('direction') == 'bullish'
                 and z.get('mid', 0) < entry_price - sl_dist * 3.0],
                key=lambda z: z.get('mid', 0), reverse=True)
            if below_1h:
                tp2 = below_1h[0]['mid']; tp2_label = below_1h[0].get('label', '1H 需求區')
        if not tp2:
            tp2 = entry_price - sl_dist * 3.5; tp2_label = "1:3.5 RR 目標"
